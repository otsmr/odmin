...
const hostname = config.get("webauthn:rp_id");
const jwt_secret = config.get("webauthn:jwt_secret");

import {
    updateSignCountBycredentialID,
    getCredentialByID
 ...
 */
export const makeCredential = (attestation) => {
    ...
    const clientData = validateClientData(C, "webauthn.create");
    ...
    const authenticatorData: any = parseAuthenticatorData(attestationObject.authData);
...
export const verifyAssertion = async (assertion) => {
    ...
    try {
        const data = await getCredentialByID(assertion.id);
        if (typeof data.publicKeyJwk !== "object") data.publicKeyJwk = JSON.parse(data.publicKeyJwk);
        fromDB = {
            userid: data.user_id,
            credential: {
        ...
        log.error("fideo", "credential: " + error.toString());
        log.error("fideo", "credential: " + assertion.toString());
        throw new Error("1 Could not find credential with that ID");
    }

    const {credential, userid} = fromDB;

    if (!credential) {
        throw new Error("2 Could not find credential with that ID");
    }
    ...
    const clientData = validateClientData(C, "webauthn.get");

    if (clientData.userid !== userid)
        throw new Error("Not the right User.");

    //Parse authenticator data used for the next few steps
    const authenticatorData: any = parseAuthenticatorData(authData);
    ...
    await updateSignCountBycredentialID(assertion.id, authenticatorData.signCount);
...
const parseAuthenticatorData = authData => {
    try {
        ...
        if (authenticatorData.flags & 64) {
            const attestedCredentialData: any = {};
